...
if size(Plan.Beams,2) > 1
  ...
  for b = 1: size(Plan.Beams,2) %Loop for each beam
    ...
    if NbSlabs > 0
        ...
        Plan.Beams(b).RSinfo.SlabOffset = param.snout.RangeShifterOffset(1:Plan.Beams(b).RSinfo.NbSlabs) - param.snout.RangeShifterOffset(Plan.Beams(b).RSinfo.NbSlabs) ; %Offset between the last slab and i-th slab

        %Compute isocentre to range shifter distance
        Plan.Beams(b).RSinfo.IsocenterToRangeShifterDistance = getIsocenterToRangeShifterDistance(Plan.Beams(b) , Plan.ScannerDirectory , Plan.BDL);
